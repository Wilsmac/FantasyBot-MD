...
const smsAvisoAG = () => { return `╔═ ≪ *•𝙰𝙳𝚅𝙴𝚁𝚃𝙴𝙽𝙲𝙸𝙰•* ≫ ═╗\n\n` }
const smsAvisoIIG = () => { return `╔═ ≪ *•𝙸𝙽𝙵𝙾𝚁𝙼𝙰𝙲𝙸́𝙾𝙽•* ≫ ═╗\n\n` }
const smsAvisoFG = () => { return `╔═ ≪ *•𝙴𝚁𝚁𝙾𝚁•* ≫ ═╗\n\n` }
const smsAvisoMG = () => { return `╔═ ≪ *•𝙰𝙳𝚅𝙴𝚁𝚃𝙴𝙽𝙲𝙸𝙰•* ≫ ═╗\n\n` }
const smsAvisoEEG = () => { return `۩ *𝚁𝙴𝙿𝙾𝚁𝚃𝙴* ۩\n\n` }
const smsAvisoEG = () => { return `*•——◤𝙴𝚇𝙸́𝚃𝙾◥——•*\n\n` }
...
const smsVersion = () => { return `Versión ❯ ${global.packname}`}
...
const smsResultPareja = () => { return `le falta una Pareja`}
...
const smsLista1 = () => { return `🌟 INFORMACIÓN DEL BOT 🌟`}
...
const smsLista7 = () => { return ` 𝐼𝑁𝑆𝑇𝐴𝐿𝐴𝑅 𝑇𝑂𝐾𝐼𝑇𝑂𝐵𝑂𝑇 `}
...
//main.js
const smsWelcome = () => { return `*╭━───≪@subject≫─━━━╮*\n*┃ 𝑩𝑰𝑬𝑵𝑽𝑬𝑵𝑰𝑫𝑨*\n┝♥️ @user\n┝♦️ *𝙻𝙴𝙰 𝙻𝙰 𝙳𝙴𝚂𝙲𝚁𝙸𝙿𝙲𝙸́𝙾𝙽 𝙳𝙴𝙻  𝙶𝙵𝚄𝙿𝙾*\n╰━─━──━━─≪✠≫─━─━━─━╯\n${String.fromCharCode(8206).repeat(850)}\n@desc`}
const smsBye = () => { return '*╭┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈⊰*\n┊ @user\n┊ *𝚄𝙽 𝙸𝙽𝚃𝙴𝙶𝚁𝙰𝙽𝚃𝙴 𝙼𝙴𝙽𝙾𝚂😿\n*╰┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈⊰*'}
const smsSpromote = () => { return '*@user 𝙰𝙷𝙾𝚁𝙰 𝙴𝚂 𝙰𝙳𝙼𝙸𝙽 𝙳𝙴𝙻 𝙶𝚁𝚄𝙿𝙾!*'}
const smsSdemote = () => { return '*@user 𝙳𝙴𝙹𝙰 𝙳𝙴 𝚂𝙴𝚁 𝙰𝙳𝙼𝙸𝙽 𝙴𝙽 𝙴𝚂𝚃𝙴 𝙶𝚁𝚄𝙿𝙾!*'}
...
const smsConexion = () => { return `\n╔═══════❖•ೋ° °ೋ•❖═══════╗\n│\n│ｃօ𝖓𝖊𝓍ⅈօ́𝖓 𝞔𐊴Ɩ𝚃૦𝘚𝔸 𝔸𐐛 𝓦Η𝔸𝚃𝘚𝔸ⲢⲢ 😺\n│\n╚═══════❖•ೋ° °ೋ•❖═══════╝ ✦┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈ `} 
const smsCargando = () => { return `🔰 𝙲𝙰𝚁𝙶𝙰𝙽𝙳𝙾...\n`} 
const smsCodigoQR = () => { return `\n ESCANEA EL CÓDIGO QR EXPIRA EN 45 SEGUNDOS ☑️`}
...
//_allantilink.js
const smsTextoYT = () => { return ' 𝑆𝑈𝑃𝐸𝑅 𝑇𝑂𝐾𝐼𝑇𝑂 - 𝗪𝗵𝗮𝘁𝘀𝗔𝗽𝗽 '} 
...
//handler.js
const smsCont1 = () => { return `*𝙲𝙾𝙼𝙰𝙽𝙳𝙾 𝙵𝙰𝙻𝙻𝙰𝙽𝙳𝙾*`}
const smsCont2 = () => { return `*⚠️ 𝙿𝙻𝚄𝙶𝙸𝙽:*`}
const smsCont3 = () => { return `*⚠️ 𝚄𝚂𝚄𝙰𝚁𝙸𝙾:*`}
const smsCont4 = () => { return `*⚠️ 𝙲𝙾𝙼𝙰𝙽𝙳𝙾:*`}
const smsCont5 = () => { return `*⚠️ 𝙴𝚁𝚁𝙾𝚁:*`}
const smsCont6 = () => { return `*❗ 𝚁𝙴𝙿𝙾𝚁𝚃𝙴 𝙴𝚂𝚃𝙴 𝙼𝙴𝙽𝚂𝙰𝙹𝙴 𝚄𝚂𝙰𝙽𝙳𝙾 𝙴𝙻 𝙲𝙾𝙼𝙰𝙽𝙳𝙾 #reporte 𝙿𝙰𝚁𝙰 𝚂𝙾𝙻𝚄𝙲𝙸𝙾𝙽𝙰𝚁𝙻𝙾*`}
const smsCont7 = () => { return `${global.lenguajeGB['smsAvisoAG']()}𝙽𝙾 𝚃𝙸𝙴𝙽𝙴𝚂 𝙳𝙸𝙰𝙼𝙰𝙽𝚃𝙴𝚂, 𝙿𝙰𝚁𝙰 𝙲𝙾𝙼𝙿𝚁𝙰𝚁 𝚄𝚂𝙰 𝙴𝙻 𝙲𝙾𝙼𝙰𝙽𝙳𝙾`}
...
const eGataCoins = () => { return '𝑭𝒂𝒏𝒕𝒂𝒔𝒚𝑪𝒐𝒊𝒏𝒔' }
...
const smsWel2 = () => { return `𝙼𝚎𝚗𝚜𝚊𝚓𝚎 𝚍𝚎 𝙱𝚒𝚎𝚗𝚟𝚎𝚗𝚒𝚍𝚊 𝚙𝚊𝚛𝚊 𝚗𝚞𝚎𝚟𝚘𝚜 𝙼𝚒𝚎𝚖𝚋𝚛𝚘𝚜 𝚎𝚗 𝙶𝚛𝚞𝚙𝚘𝚜`}
const smsDete1 = () => { return `🔔 𝙰𝚅𝙸𝚂𝙾𝚂`}
const smsDete2 = () => { return `𝙰𝚟𝚒𝚜𝚘𝚜 𝚍𝚎 𝚊𝚌𝚌𝚒𝚘𝚗𝚎𝚜 𝚍𝚎𝚗𝚝𝚛𝚘 𝚍𝚎𝚕 𝙶𝚛𝚞𝚙𝚘`}
const smsANivel1 = () => { return `🆙 NIVEL AUTOMÁTICO`}
const smsANivel2 = () => { return `𝚂𝚞𝚋𝚎 𝚍𝚎 𝚗𝚒𝚟𝚎𝚕 𝚊 𝚝𝚘𝚍𝚘𝚜 𝚍𝚎 𝚖𝚊𝚗𝚎𝚛𝚊 𝚊𝚞𝚝𝚘𝚖𝚊𝚝𝚒𝚌𝚊; (𝙰𝚙𝚕𝚒𝚌𝚊 𝚛𝚎𝚌𝚘𝚖𝚙𝚎𝚗𝚜𝚊𝚜 𝚙𝚘𝚛 𝚜𝚞𝚋𝚒𝚛 𝚍𝚎 𝙽𝚒𝚟𝚎𝚕)`}
const smsRestri1 = () => { return `⛔ 𝚁𝙴𝚂𝚃𝚁𝙸𝙽𝙶𝙸𝚁`}
const smsRestri2 = () => { return `𝙷𝚊𝚋𝚒𝚕𝚒𝚝𝚊𝚛 𝙵𝚄𝙽𝙲𝙸𝙾́𝙽 𝚙𝚊𝚛𝚊 𝚊𝚐𝚛𝚎𝚐𝚊𝚛 𝚘 𝚎𝚕𝚒𝚖𝚒𝚗𝚊𝚛 𝚙𝚎𝚛𝚜𝚘𝚗𝚊𝚜 𝚎𝚗 𝙶𝚛𝚞𝚙𝚘𝚜`}
const smsLlamar1 = () => { return `🚫 𝙰𝙽𝚃𝙸 𝙻𝙻𝙰𝙼𝙰𝙳𝙰𝚂`}
const smsLlamar2 = () => { return `𝙱𝚕𝚘𝚚𝚞𝚎𝚊 𝚊 𝙿𝚎𝚛𝚜𝚘𝚗𝚊𝚜 𝚚𝚞𝚎 𝚑𝚊𝚐𝚊𝚗 𝚕𝚕𝚊𝚖𝚊𝚍𝚊𝚜`}
const smsAntiSp1 = () => { return `🚯 𝙰𝙽𝚃𝙸 𝚂𝙿𝙰𝙼`}
const smsAntiSp2 = () => { return `𝙱𝚊𝚗𝚎𝚊𝚛 𝚎𝚕 𝚄𝚜𝚘 𝚍𝚎 𝙲𝚘𝚖𝚊𝚍𝚘𝚜 𝚌𝚞𝚊𝚗𝚍𝚘 𝚊𝚕𝚐𝚞𝚒𝚎𝚗 𝚛𝚎𝚊𝚕𝚒𝚌𝚎 𝚊𝚕𝚐𝚞́𝚗 𝚝𝚒𝚙𝚘 𝚍𝚎 𝚂𝚙𝚊𝚖`}
const smsModP1 = () => { return `🌐 𝙼𝙾𝙳𝙾 𝙿𝚄́𝙱𝙻𝙸𝙲𝙾`}
const smsModP2 = () => { return `Habilitar función para que todos puedan usar FantasyBot-MD`}
const smsModAd1 = () => { return `🛂 𝙼𝙾𝙳𝙾 𝙰𝙳𝙼𝙸𝙽`}
const smsModAd2 = () => { return `𝚂𝚘𝚕𝚘 𝚕𝚘𝚜 𝙰𝚍𝚖𝚒𝚗𝚜 𝚙𝚘𝚍𝚛 usar 𝚎𝚕 𝚋𝚘𝚝 𝚎𝚗 𝙶𝚛𝚞𝚙𝚘𝚜`}
const smsLect1 = () => { return `✅ 𝙻𝙴𝙲𝚃𝚄𝚁𝙰 𝙰𝚄́𝚃𝙾𝙼𝙰𝚃𝙸𝙲𝙰`}
const smsLect2 = () => { return `𝙳𝚎𝚓𝚊𝚛 𝚕𝚘𝚜 𝚖𝚎𝚗𝚜𝚊𝚓𝚎𝚜 𝚘 𝚌𝚑𝚊𝚝𝚜 𝚌𝚘𝚖𝚘 𝙻𝚎𝚒́𝚍𝚘𝚜`}
const smsTempo1 = () => { return ` 𝙱𝙾𝚃 𝚃𝙴𝙼𝙿𝙾𝚁𝙰𝙻`}
const smsTempo2 = () => { return `𝙵𝚞𝚗𝚌𝚒𝚘́𝚗 𝚚𝚞𝚎 𝚙𝚎𝚛𝚖𝚒𝚝𝚎 𝙴𝚜𝚝𝚊𝚍𝚒́𝚊 𝚝𝚎𝚖𝚙𝚘𝚛𝚊𝚕𝚖𝚎𝚗𝚝𝚎 𝚎𝚗 𝙶𝚛𝚞𝚙𝚘𝚜`}
const smsStik1 = () => { return `🎠 𝚂𝚃𝙸𝙲𝙺𝙴𝚁𝚂`}
const smsStik2 = () => { return `𝙷𝚊𝚋𝚒𝚕𝚒𝚝𝚊𝚛 𝚎𝚕 𝚎𝚗𝚟𝚒𝚘 automático de Stickers a todos`}
const smsStickA1 = () => { return `🪄 𝚂𝚃𝙸𝙲𝙺𝙴𝚁𝚂 AUTOMÁ𝚃𝙸𝙲𝙾𝚂`}
const smsStickA2 = () => { return `𝙻𝚘𝚜 𝚟𝚒𝚍𝚎𝚘𝚜, 𝙶𝚒𝚏, 𝚒𝚖𝚊𝚐𝚎𝚗𝚎𝚜, 𝚎𝚗𝚕𝚊𝚌𝚎𝚜 𝚓𝚙𝚐 𝚘 𝚓𝚙𝚎𝚐; 𝚂𝚎 𝚌𝚘𝚗𝚟𝚎𝚛𝚝𝚒𝚛𝚊𝚗 𝚎𝚗 𝚂𝚝𝚒𝚌𝚔𝚎𝚛𝚜 𝙰𝚞𝚝𝚘𝚖𝚊𝚝𝚒𝚌𝚊𝚖𝚎𝚗𝚝𝚎`}
const smsReacc1 = () => { return `𝚁𝙴𝙰𝙲𝙲𝙸́𝙾𝙽`}
const smsReacc2 = () => { return `𝙷𝚊𝚋𝚒𝚕𝚒𝚝𝚊𝚛 𝚎𝚕 𝚎𝚗𝚟𝚒𝚘 𝚊𝚞𝚝𝚘𝚖𝚊𝚝𝚒𝚌𝚘 𝚍𝚎 𝚁𝚎𝚊𝚌𝚌𝚒𝚘𝚗𝚎𝚜 𝚊 𝚖𝚎𝚗𝚜𝚊𝚓𝚎𝚜`}
const smsAudi1 = () => { return `🔊 𝙰𝚄𝙳𝙸𝙾𝚂`}
const smsAudi2 = () => { return `𝙷𝚊𝚋𝚒𝚕𝚒𝚝𝚊𝚛 𝚎𝚕 𝚎𝚗𝚟𝚒𝚘 𝚊𝚞𝚝𝚘𝚖𝚊𝚝𝚒𝚌𝚘 𝚍𝚎 𝙰𝚞𝚍𝚒𝚘𝚜 𝚊 𝚝𝚘𝚍𝚘𝚜`}
const smsModHor1 = () => { return `🔞 𝙼𝙾𝙳𝙾 𝙷𝙾𝚁𝙽𝚈`}
const smsModHor2 = () => { return `𝙼𝚘𝚜𝚝𝚛𝚊𝚛 𝚌𝚘𝚗𝚝𝚎𝚗𝚒𝚍𝚘 𝚙𝚊𝚛𝚊 𝙰𝚍𝚞𝚕𝚝𝚘 𝚎𝚗 𝚕𝚘𝚜 𝙲𝚑𝚊𝚝𝚜`}
const smsAntitoc1 = () => { return `☢️ 𝙰𝙽𝚃𝙸 𝚃𝙾́𝚇𝙸𝙲𝙾𝚂`}
const smsAntitoc2 = () => { return `𝙴𝚗𝚟𝚒𝚊𝚛 𝙰𝚍𝚟𝚎𝚛𝚝𝚎𝚗𝚌𝚒𝚊𝚜 𝚊𝚚𝚞𝚎𝚕𝚕𝚊𝚜 𝚙𝚎𝚛𝚜𝚘𝚗𝚊𝚜 𝚚𝚞𝚎 𝚒𝚗𝚜𝚞𝚕𝚝𝚎𝚗`}
const smsModOb1 = () => { return `👀 𝙼𝙾𝙳𝙾 𝙾𝙱𝚂𝙴𝚁𝚅𝙰𝚁`}
const smsModOb2 = () => { return `𝙿𝚎𝚛𝚖𝚒𝚝𝚒𝚛 𝚚𝚞𝚎 𝚕𝚊𝚜 𝚒𝚖𝚊𝚐𝚎𝚗𝚎𝚜, 𝙶𝚒𝚏 𝚢 𝚅𝚒́𝚍𝚎𝚘𝚜 𝚜𝚎 𝚙𝚞𝚎𝚍𝚊𝚗 𝚟𝚎𝚛 𝚙𝚊𝚛𝚊 𝚝𝚘𝚍𝚘𝚜`}
const smsAntiEli1 = () => { return `🗑️ 𝙰𝙽𝚃𝙸 𝙴𝙻𝙸𝙼𝙸𝙽𝙰𝚁`}
const smsAntiEli2 = () => { return `𝚃𝚘𝚍𝚘 𝚖𝚎𝚗𝚜𝚊𝚓𝚎 𝚎𝚕𝚒𝚖𝚒𝚗𝚊𝚍𝚘 𝚜𝚎𝚛𝚊 𝚛𝚎𝚎𝚗𝚟𝚒𝚊𝚍𝚘 𝚊𝚕 𝙲𝚑𝚊𝚝 𝚘 𝙶𝚛𝚞𝚙𝚘`}
const smsAntiInt1 = () => { return `🌏 𝙰𝙽𝚃𝙸 𝙸𝙽𝚃𝙴𝚁𝙽𝙰𝙲𝙸𝙾𝙽𝙰𝙻`}
const smsAntiInt2 = () => { return `𝙴𝚕𝚒𝚖𝚒𝚗𝚊𝚛 𝙽𝚞𝚖𝚎𝚛𝚘𝚜 𝚒𝚗𝚝𝚎𝚛𝚗𝚊𝚌𝚒𝚘𝚗𝚊𝚕𝚎𝚜 𝚌𝚘𝚗𝚜𝚒𝚍𝚎𝚛𝚊𝚍𝚘𝚜 𝚏𝚊𝚕𝚜𝚘𝚜`}
const smsAntiE1 = () => { return `🔗 𝙰𝙽𝚃𝙸 𝙴𝙽𝙻𝙰𝙲𝙴𝚂`}
const smsAntiE2 = () => { return `𝙴𝚕𝚒𝚖𝚒𝚗𝚊𝚛 𝙿𝚎𝚛𝚜𝚘𝚗𝚊𝚜 𝚚𝚞𝚎 𝚎𝚗𝚟𝚒𝚎𝚗 𝚎𝚗𝚕𝚊𝚌𝚎𝚜 𝚍𝚎 𝙶𝚛𝚞𝚙𝚘𝚜 𝚍𝚎 𝚆𝚑𝚊𝚝𝚜𝙰𝚙𝚙`}
const smsAntiEE1 = () => { return `🔗 ANTI ENLACES 2`}
const smsAntiEE2 = () => { return `𝙴𝚕𝚒𝚖𝚒𝚗𝚊𝚛 𝙿𝚎𝚛𝚜𝚘𝚗𝚊𝚜 𝚚𝚞𝚎 𝚎𝚗𝚟𝚒𝚎𝚗 𝚎𝚗𝚕𝚊𝚌𝚎𝚜 𝚚𝚞𝚎 𝚌𝚘𝚗𝚝𝚎𝚗𝚐𝚊𝚗 𝚑𝚝𝚝𝚙𝚜`}
const smsAntiTT1 = () => { return `🔗 𝙰𝙽𝚃𝙸 𝚃𝙸𝙺𝚃𝙾𝙺`}
const smsAntiTT2 = () => { return `𝙴𝚕𝚒𝚖𝚒𝚗𝚊𝚛 𝙿𝚎𝚛𝚜𝚘𝚗𝚊𝚜 𝚚𝚞𝚎 𝚎𝚗𝚟𝚒𝚎𝚗 𝚎𝚗𝚕𝚊𝚌𝚎𝚜 𝚍𝚎 𝚃𝚒𝚔𝚃𝚘𝚔`}
const smsAntiYT1 = () => { return `🔗 𝙰𝙽𝚃𝙸 𝚈𝙾𝚄𝚃𝚄𝙱𝙴`}
const smsAntiYT2 = () => { return `𝙴𝚕𝚒𝚖𝚒𝚗𝚊𝚛 𝙿𝚎𝚛𝚜𝚘𝚗𝚊𝚜 𝚚𝚞𝚎 𝚎𝚗𝚟𝚒𝚎𝚗 𝚎𝚗𝚕𝚊𝚌𝚎𝚜 𝚍𝚎 𝚈𝚘𝚞𝚃𝚞𝚋𝚎`}
const smsAntiTEL1 = () => { return `🔗 𝙰𝙽𝚃𝙸 𝚃𝙴𝙻𝙴𝙶𝚁𝙰𝙼`}
const smsAntiTEL2 = () => { return `𝙴𝚕𝚒𝚖𝚒𝚗𝚊𝚛 𝙿𝚎𝚛𝚜𝚘𝚗𝚊𝚜 𝚚𝚞𝚎 𝚎𝚗𝚟𝚒𝚎𝚗 𝚎𝚗𝚕𝚊𝚌𝚎𝚜 𝚍𝚎 𝚃𝚎𝚕𝚎𝚐𝚛𝚊𝚖`}
const smsAntiFB1 = () => { return `🔗 𝙰𝙽𝚃𝙸 𝙵𝙰𝙲𝙴𝙱𝙾𝙾𝙺`}
const smsAntiFB2 = () => { return `𝙴𝚕𝚒𝚖𝚒𝚗𝚊𝚛 𝙿𝚎𝚛𝚜𝚘𝚗𝚊𝚜 𝚚𝚞𝚎 𝚎𝚗𝚟𝚒𝚎𝚗 𝚎𝚗𝚕𝚊𝚌𝚎𝚜 𝚍𝚎 𝙵𝚊𝚌𝚎𝚋𝚋𝚘𝚔`}
const smsAntiIG1 = () => { return `🔗 𝙰𝙽𝚃𝙸 𝙸𝙽𝚂𝚃𝙰𝙶𝚁𝙰𝙼`}
const smsAntiIG2 = () => { return `𝙴𝚕𝚒𝚖𝚒𝚗𝚊𝚛 𝙿𝚎𝚛𝚜𝚘𝚗𝚊𝚜 𝚚𝚞𝚎 𝚎𝚗𝚟𝚒𝚎𝚗 𝚎𝚗𝚕𝚊𝚌𝚎𝚜 𝚍𝚎 𝙸𝚗𝚜𝚝𝚊𝚐𝚛𝚊𝚖`}
const smsAntiTW1 = () => { return `🔗 𝙰𝙽𝚃𝙸 𝚃𝚆𝙸𝚃𝚃𝙴𝚁 `}
const smsAntiTW2 = () => { return `𝙴𝚕𝚒𝚖𝚒𝚗𝚊𝚛 𝙿𝚎𝚛𝚜𝚘𝚗𝚊𝚜 𝚚𝚞𝚎 𝚎𝚗𝚟𝚒𝚎𝚗 𝚎𝚗𝚕𝚊𝚌𝚎𝚜 𝚍𝚎 𝚃𝚠𝚒𝚝𝚝𝚎𝚛`}
const smsSOLOP1 = () => { return `⚜️ 𝚂𝙾𝙻𝙾 𝙿𝚁𝙸𝚅𝙰𝙳𝙾𝚂`}
const smsSOLOP2 = () => { return `𝙿𝚎𝚛𝚖𝚒𝚝𝚒𝚛 𝚚𝚞𝚎 𝚜𝚘𝚕𝚘 𝚜𝚎 𝚞𝚜𝚎 𝚎𝚗 𝙲𝚑𝚊𝚝𝚜 𝙿𝚛𝚒𝚟𝚊𝚍𝚘𝚜`}
const smsSOLOG1 = () => { return `⚜️ 𝚂𝙾𝙻𝙾 𝙶𝚁𝚄𝙿𝙾𝚂`}
const smsSOLOG2 = () => { return `𝙿𝚎𝚛𝚖𝚒𝚝𝚒𝚛 𝚚𝚞𝚎 𝚜𝚘𝚕𝚘 𝚜𝚎 𝚞𝚜𝚎 𝚎𝚗 𝙲𝚑𝚊𝚝𝚜 𝙶𝚛𝚞𝚙𝚊𝚕𝚎𝚜`}
const smsConfi1 = () => { return `𝙰𝙹𝚄𝚂𝚃𝙴𝚂`}
const smsConfi2 = () => { return `*¡𝙷𝚘𝚕𝚊!*`}
const smsConfi3 = () => { return `*𝚂𝚎𝚕𝚎𝚌𝚌𝚒𝚘𝚗𝚎 𝚞𝚗𝚊 𝚘𝚙𝚌𝚒𝚘́𝚗 𝚍𝚎 𝚕𝚊 𝚕𝚒𝚜𝚝𝚊*`}
const smsConfi4 = () => { return `*𝙿𝚊𝚛𝚊 𝚎𝚖𝚙𝚎𝚣𝚊𝚛 𝚊 𝙲𝚘𝚗𝚏𝚒𝚐𝚞𝚛𝚊𝚛*`}
const smsConfi5 = () => { return `● *𝙰𝚟𝚒𝚜𝚘𝚜 𝚍𝚎 𝚕𝚊 𝙲𝚘𝚗𝚏𝚒𝚐𝚞𝚛𝚊𝚌𝚒𝚘𝚗:*`}
const smsConfi6 = () => { return `✅ ⇢ *𝙵𝚞𝚗𝚌𝚒𝚘́𝚗 𝙰𝚌𝚝𝚒𝚟𝚊𝚍𝚊 ^^*`}
const smsConfi7 = () => { return `❌ ⇢ *𝙵𝚞𝚗𝚌𝚒𝚘́𝚗 𝙳𝚎𝚜𝚊𝚌𝚝𝚒𝚟𝚊𝚍𝚊*`}
const smsConfi8 = () => { return `⚠️ ⇢ *𝙴́𝚜𝚝𝚎 𝙲𝚑𝚊𝚝 𝚗𝚘 𝚎𝚜 𝚞𝚗 𝙶𝚛𝚞𝚙𝚘*`}
...
const smsBCbot7 = () => { return `✅ *𝑪𝑶𝑴𝑼𝑵𝑰𝑪𝑨𝑫𝑶 𝑶𝑭𝑰𝑪𝑰𝑨𝑳* ✅`}
...
const smsJBPrincipal = () => { return `${lenguajeGB['smsAvisoAG']()}🔵 *𝙿𝙰𝚁𝙰 𝚂𝙴𝚁 𝚂𝚄?𝙱𝙾𝙾𝚃 𝙳𝙸𝚁𝙸𝙹𝙰𝚂𝙴 𝙰𝙻 𝙽𝚄́𝙼𝙴𝚁𝙾 𝙿𝚁𝙸𝙽𝙲𝙸𝙿𝙰𝙻*\n*⟴ Ingrese al siguiente enlace:*\n`}
...
const smsJBConexionClose2 = () => { return `${lenguajeGB['smsAvisoFG']()}🔴 *𝙻𝙰 𝚌𝚘𝚗𝚎𝚡𝚒𝚘́𝚗 𝚜𝚎 𝚑𝚊 𝚌𝚎𝚛𝚛𝚊𝚍𝚘, TENDRAS QUE VOLVER A CONECTARSE USANDO:*\n#deletesesion (Para borrar los datos y poder volver a solita el QR o el code)`}
const smsJBConexionTrue = () => { return `${lenguajeGB['smsAvisoEG']()}🌺 𝚌𝚘𝚗𝚎𝚡𝚒𝚘́𝚗 𝚌𝚘𝚗 𝙴́𝚎́𝚡𝚒𝚝𝚘!*`}
const smsJBConexionTrue2 = () => { return `${lenguajeGB['smsAvisoEG']()}🟢 *𝙲𝙾𝙽𝙴𝚇𝙸𝙾𝙽 𝙲𝙾𝙽 𝙴́𝚇𝙸𝚃𝙾! 𝙿𝚄𝙴𝙳𝙴 𝙲𝙾𝙽𝙴𝙲𝚃𝙰𝚁𝚂𝙴 𝚄𝚂𝙰𝙽𝙳𝙾 𝚂𝚄 (ID) 𝙾 𝚄𝚂𝙰𝚁:*`}
const smsJBCargando = () => { return `${lenguajeGB['smsAvisoIIG']()}🌺 *𝙴𝚂𝚃𝙰́ 𝙲𝙾𝙽𝙴𝙲𝚃𝙰𝙳𝙾 𝙿𝙾𝚁 𝙵𝙰𝚅𝙾𝚁 𝙴𝚂𝙿𝙴𝚁𝙴 𝚂𝙴 𝙴𝚂𝚃𝙰́ 𝙲𝙰𝚁𝙶𝙰𝙽𝙳𝙾 𝙻𝙾𝚂 𝙼𝙴𝙽𝚂𝙰𝙹𝙴𝚂...*\n\n♻️ *𝙾𝙿𝙲𝙸𝙾𝙽𝙴𝚂 𝙳𝙸𝚂𝙿𝙾𝙽𝙸𝙱𝙻𝙴𝚂:*\n*» #stop _(𝙳𝚎𝚝𝚎𝚗𝚎𝚛 𝚕𝚊 𝚏𝚞𝚗𝚌𝚒𝚘́𝚗 𝚂𝚞𝚋 𝙱𝚘𝚝)_*\n*» #eliminarsesion _(Borrar todo rastro de Sub Bot)_*\n*» #serbot _(Obtener nuevo código QR para ser Sub Bot)_*`}
const smsJBInfo1 = () => { return `💖 *ENLACE ÚTIL*`}
const smsJBInfo2 = () => { return `💖 *La función es estable, sí presenta algún inconveniente Comuníquese al correo: indefinido202@gmail.com*\n💝 *Puede hacer una Donación voluntaria por PayPal: ${global.paypal}*\n\n*Muchas Gracias por el apoyo a ${global.packname}*`}
